...
Admin:
	...
	if BConfig.Listen.EnableAdmin {
		...
		if FilterMonitorFunc(invokeCtx.request.Method, invokeCtx.request.URL.Path, timeDur, pattern, statusCode) {
			if invokeCtx.runRouter != nil {
				go toolbox.StatisticsMap.AddStatistics(invokeCtx.request.Method, invokeCtx.request.URL.Path, invokeCtx.runRouter.Name(), timeDur)
			} else {
				go toolbox.StatisticsMap.AddStatistics(invokeCtx.request.Method, invokeCtx.request.URL.Path, "", timeDur)
			}
		}
	}
	...
	if BConfig.RunMode == DEV && !BConfig.Log.AccessLogs {
		...
		methodColor := logs.ColorByMethod(iswin, invokeCtx.request.Method)
		...
		if invokeCtx.findRouter {
			if invokeCtx.routerInfo != nil {
				devInfo = fmt.Sprintf("|%15s|%s %3d %s|%13s|%8s|%s %-7s %s %-3s   r:%s", context.Input.IP(), statusColor, statusCode,
					resetColor, timeDur.String(), "match", methodColor, invokeCtx.request.Method, resetColor, invokeCtx.request.URL.Path,
					invokeCtx.routerInfo.pattern)
			} else {
				devInfo = fmt.Sprintf("|%15s|%s %3d %s|%13s|%8s|%s %-7s %s %-3s", context.Input.IP(), statusColor, statusCode, resetColor,
					timeDur.String(), "match", methodColor, invokeCtx.request.Method, resetColor, invokeCtx.request.URL.Path)
			}
		} else {
			devInfo = fmt.Sprintf("|%15s|%s %3d %s|%13s|%8s|%s %-7s %s %-3s", context.Input.IP(), statusColor, statusCode, resetColor,
				timeDur.String(), "nomatch", methodColor, invokeCtx.request.Method, resetColor, invokeCtx.request.URL.Path)
		}
		if iswin {
